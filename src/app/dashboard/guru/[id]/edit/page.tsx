...
export default function EditGuruPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchData = async () => {
      try {
        ...
        Object.keys(json).forEach((key) => {
          // Tanggal sudah akan dihandle oleh DatePicker yang menerima Date object atau string
          setValue(key as keyof GuruInput, json[key]);
        });
      } catch (err) {
  ...
  const onSubmit = async (data: GuruInput) => {
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Gagal mengupdate data');
    ...
    <div className="min-h-screen bg-slate-50 pb-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4 pt-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Edit Data Guru</h1>
            <p className="text-slate-500 text-sm">Perbarui informasi guru dan staf</p>
          </div>
        </div>
